...
public record InitializeOrder(Guid OrderId, Guid ClientId, IReadOnlyList<PricedProductItem> ProductItems, decimal TotalPrice): ICommand
{
    public static InitializeOrder Create(Guid? orderId, Guid? clientId, IReadOnlyList<PricedProductItem>? productItems, decimal? totalPrice)
    {
        if (!orderId.HasValue)
...
public class HandleInitializeOrder: ICommandHandler<InitializeOrder>
{
    ...
    public HandleInitializeOrder(IMartenRepository<Order> orderRepository, IMartenAppendScope scope)
    {
    ...
    public async Task<Unit> Handle(InitializeOrder command, CancellationToken cancellationToken)
    {
        ...
        await scope.Do((_, eventMetadata) =>
            orderRepository.Add(
                ...
                eventMetadata, cancellationToken)
        );
        return Unit.Value;
    }
}
