...
public class Reservation : Aggregate
{
    ...
    public static Reservation CreateTentative(Guid id, IReservationNumberGenerator numberGenerator, Guid seatId)
    {
        return new Reservation(id, numberGenerator, seatId);
    }

    private Reservation(Guid id, IReservationNumberGenerator numberGenerator, Guid seatId)
    {
        if (id == Guid.Empty)
